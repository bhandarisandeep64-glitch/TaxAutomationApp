...
export default function TdsZoho() {
  ...
  const [finalFileName, setFinalFileName] = useState(''); // New State
  ...
  const handleRunAutomation = async () => {
    ...
    try {
      const response = await fetch('http://127.0.0.1:5000/api/direct-tax/tds-zoho', {
        method: 'POST',
        body: formData,
      });
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex items-center gap-3 border-b border-slate-800 pb-4">
        ...
        <h2 className="text-xl font-bold text-white tracking-tight">TDS Automation <span className="text-slate-600">|</span> Zoho</h2>
      </div>
      ...
      {status === 'success' && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 animate-in slide-in-from-bottom-4 duration-700">
          ...
          <div className={`md:col-span-1 ${THEME.card} border ${THEME.border} p-6 rounded-xl flex flex-col items-center justify-center text-center relative overflow-hidden`}>
             ...
             <a 
                href={downloadUrl} 
                download={finalFileName} // Force filename
                className="w-full mt-4 flex items-center justify-center gap-2 px-4 py-2 bg-green-600 hover:bg-green-500 text-white text-sm font-bold rounded-lg transition-all shadow-lg shadow-green-900/20"
             >
