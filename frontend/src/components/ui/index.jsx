...
export function PageHeader({ icon: Icon, title, eyebrow, subtitle, action }) {
  return (
    <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 border-b border-white/[0.07] pb-6">
      <div className="flex items-center gap-4">
        {Icon && (
          <div className="w-11 h-11 rounded-lg bg-neutral-900 border border-white/[0.07] flex items-center justify-center shrink-0">
...
export function Card({ children, className = '', padded = true }) {
  return (
    <div className={`rounded-lg ${THEME.card} ${padded ? 'p-6' : ''} ${className}`}>
      {children}
    </div>
  );
}
...
export function Button({ variant = 'primary', icon: Icon, loading, children, className = '', ...props }) {
  return (
    <button
      className={`inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-md text-sm ${BUTTON_VARIANTS[variant]} ${className}`}
...
export function StatCard({ icon: Icon, label, value, sub }) {
  return (
    <div className={`p-6 rounded-lg ${THEME.card} relative overflow-hidden group`}>
      <div className="absolute top-0 right-0 p-4 opacity-[0.05] group-hover:opacity-[0.08] transition-opacity">
        <Icon className="w-20 h-20 text-indigo-400" strokeWidth={1} />
      </div>
...
export function EmptyState({ icon: Icon, title, subtitle }) {
  return (
    <div className="flex flex-col items-center justify-center text-center py-16 px-6">
      {Icon && (
        <div className="w-12 h-12 rounded-full bg-neutral-900 border border-white/[0.07] flex items-center justify-center mb-4">
...
export function Spinner({ className = 'w-5 h-5' }) {
  return <Loader2 className={`${className} animate-spin text-indigo-400`} />;
}
...
export function UploadSlot({ title, file, onChange, onRemove, accept = '.xlsx, .csv', icon: FileIcon }) {
  return (
    <div className={`relative border-2 border-dashed rounded-lg p-4 flex flex-col justify-center items-center text-center h-32 transition-colors ${file ? 'border-emerald-500/40 bg-emerald-500/[0.04]' : 'border-white/[0.1] bg-black/20 hover:border-indigo-500/40'}`}>
