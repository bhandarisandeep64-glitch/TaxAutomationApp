...
import { FileText, CheckCircle, Download, X, Plus, TrendingUp, Zap } from 'lucide-react';
...
export default function MarioSales() {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 animate-in fade-in duration-500">
      <PageHeader icon={TrendingUp} eyebrow="Mario" title="Sales Converter" subtitle="Odoo" />

      <Card>
        <div className="flex flex-col lg:flex-row gap-8 items-stretch">
          <div className="flex-1 w-full grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="space-y-4">
              <h3 className="text-sm font-semibold text-neutral-200 border-b border-white/[0.06] pb-2">B2B Invoices</h3>
              {renderFileSlot('B2B CGST', 'file_b2b_cgst')}
              {renderFileSlot('B2B IGST', 'file_b2b_igst')}
            </div>

            <div className="space-y-4">
              <h3 className="text-sm font-semibold text-neutral-200 border-b border-white/[0.06] pb-2">B2C Invoices</h3>
              {renderFileSlot('B2C CGST', 'file_b2c_cgst')}
              {renderFileSlot('B2C IGST', 'file_b2c_igst')}
            </div>

            <div className="space-y-4">
              <h3 className="text-sm font-semibold text-neutral-200 border-b border-white/[0.06] pb-2">Credit Notes (B2B)</h3>
          ...
          <div className="flex-shrink-0 flex items-center justify-center lg:border-l border-white/[0.06] lg:pl-8 pt-6 lg:pt-0">
            <div className="flex flex-col items-center gap-3">
              <Button icon={Zap} loading={status === 'processing'} disabled={!Object.values(files).some(f => f !== null)} onClick={handleRunAutomation}>
                {status === 'processing' ? 'Processing' : 'Process Sales Data'}
              </Button>
      ...
      {status === 'success' && blobUrl && (
        <div className="grid grid-cols-1 gap-6 animate-in slide-in-from-bottom-4 duration-700">
          <Card className="w-full max-w-sm mx-auto flex flex-col items-center justify-center text-center relative overflow-hidden">
            ...
            <p className="text-sm text-neutral-500 mb-6">Your master sales file has been downloaded.</p>
            <a
              href={blobUrl}
