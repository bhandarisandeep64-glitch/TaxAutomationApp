...
# section/table-item structure with numeric TRACES codes. Some old sections
# split into multiple new codes depending on the nature of payment -- the
# "Rate at which deducted" column is used to disambiguate those (confirmed
# with the firm against their own new-section/code/rate table), since that's
# the only distinguishing signal already present in a Zoho TDS export.
#
...
def _lookup_new_section(old_section, rate):
